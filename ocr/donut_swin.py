...
from .utils import custom_roll, window_partition, window_reverse
...
class DonutSwinPatchEmbeddings(nn.Module):
    ...
    def maybe_pad(self, pixel_values, height, width):
        # Initialize pad_width with zeros for all dimensions
        pad_width = [(0, 0)] * 4
        if width % self.patch_size[1] != 0:
            pad_width[3] = (0, self.patch_size[1] - width % self.patch_size[1])
        if height % self.patch_size[0] != 0:
            pad_width[2] = (0, self.patch_size[0] - height %
                            self.patch_size[0])
        pixel_values = mx.pad(
            pixel_values, pad_width=pad_width, constant_values=0)
        return pixel_values
...
class DonutSwinSelfAttention(nn.Module):
    def __init__(self, config, dim, num_heads, window_size):
        ...
        if dim % num_heads != 0:
            raise ValueError(
                f"The hidden size ({dim}) is not a multiple of the number of attention heads ({
                    num_heads})"
            )
        ...
        coords = mx.stack(mx.meshgrid(
            coords_h, coords_w, indexing='ij'), axis=0)
        ...
        self.key = nn.Linear(self.all_head_size,
    ...
    def __call__(
        ...
        attention_scores = attention_scores / \
            mx.sqrt(float(self.attention_head_size))
        ...
        attention_scores = attention_scores + \
            mx.expand_dims(relative_position_bias, 0)
        if attention_mask is not None:
            ...
            attention_scores = attention_scores + \
                mx.expand_dims(mx.expand_dims(attention_mask, 1), 0)
            attention_scores = attention_scores.reshape(
                -1, self.num_attention_heads, dim, dim)

        attention_probs = mx.softmax(attention_scores.astype(
            mx.float32), axis=-1).astype(attention_scores.dtype)
...
class DonutSwinLayer(nn.Module):
    ...
    def maybe_pad(self, hidden_states, height, width):
        ...
        hidden_states = mx.pad(
            hidden_states, pad_width=pad_values, constant_values=0)
    ...
    def get_attn_mask(self, height, width):
        if self.shift_size > 0:
            ...
            mask_windows = mask_windows.reshape(-1,
                                                self.window_size * self.window_size)
            attn_mask = mask_windows[:, mx.newaxis,
                                     :] - mask_windows[:, :, mx.newaxis]
            attn_mask = mx.where(attn_mask != 0, -100.0,
    ...
    def __call__(
        ...
        # cyclic shift
        if self.shift_size > 0:
            shifted_hidden_states = custom_roll(
                hidden_states, shift=(-self.shift_size, -self.shift_size), axis=(1, 2))
        else:
        ...
        if self.shift_size > 0:
            attention_windows = custom_roll(shifted_windows, shift=(
...
class DonutSwinStage(nn.Module):
    ...
    def __call__(
        ...
        if self.downsample is not None:
            height_downsampled, width_downsampled = (
                height + 1) // 2, (width + 1) // 2
            output_dimensions = (
                height, width, height_downsampled, width_downsampled)
        ...
        stage_outputs = (
            hidden_states, hidden_states_before_downsampling, output_dimensions)
...
class DonutSwinPatchMerging(nn.Module):
    ...
    def maybe_pad(self, input_feature, height, width):
        ...
        if should_pad:
            ...
            input_feature = mx.pad(
                input_feature, pad_width=pad_values, constant_values=0)
    ...
    def __call__(self, input_feature: mx.array, input_dimensions: Tuple[int, int]) -> mx.array:
        ...
        input_feature = mx.concatenate(
            [input_feature_0, input_feature_1, input_feature_2, input_feature_3], axis=-1)
        input_feature = input_feature.reshape(
            batch_size, -1, 4 * num_channels)  # batch_size height/2*width/2 4*C
...
class DonutSwinEncoder(nn.Module):
    def __init__(self, config: DonutSwinModelConfig, grid_size):
        ...
        dpr = mx.linspace(start=0, stop=config.drop_path_rate,
                          num=sum(config.depths)).tolist()
        self.layers = [
            DonutSwinStage(
                config=config,
                dim=int(config.embed_dim * 2**layer_id),
                input_resolution=(
                    grid_size[0] // (2**layer_id), grid_size[1] // (2**layer_id)),
                ...
                downsample=DonutSwinPatchMerging if (
                    layer_id < self.num_layers - 1) else None,
...
class DonutSwinModel(nn.Module):
    ...
    def __call__(
        ...
        encoder_outputs = self.encoder(
            embedding_output, input_dimensions, head_mask=head_mask)
    ...
    def sanitize(weights):
        ...
        for k, v in weights.items():
            if k.startswith("decoder."):
                continue
            # mlx omits `encoder.` prefixes in k
            k = k.replace("encoder.", "", 1)
            if "projection.weight" in k:
                # !pytorch tensor shape: [out_channels, in_channels, kH, KW]
                # !mlx tensor shape: [out_channels, kH, KW, in_channels]
